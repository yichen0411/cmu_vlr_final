...
client = carla.Client('172.27.176.1', 2000)
...
def camera_callback(carla_image, rgb_image_queue):
    image_np = np.reshape(np.copy(carla_image.raw_data), (carla_image.height, carla_image.width, 4))

    cv2.imwrite("frame.jpg", image_np)
    image_pil, image = load_image("frame.jpg")
    ...
    save_path = os.path.join(output_dir, "pred.jpg")
    image_with_box.save(save_path)
    # rgb_image_queue.put(image_with_box)
